...
rule long_trim:
    input:
        long_raw=lambda wildcards: "{fastq_path}/{srr}.fastq".format(srr=name[wildcards.sample], fastq_path=fastq_dir)
    output:
...
rule kraken2:
    ...
    params:
         ...
         filter_name=lambda wildcards: " ".join(filter_name[wildcards.sample]),
    threads:8
    wildcard_constraints:
        sample="|".join(samples)
    conda:
    ...
    shell:
        ...
        for filter_name in {params.filter_name}; do
          fgrep $filter_name {output.out} | cut -f2 >> {output.readlist}
        done
        # 使用sort命令去除readlist1中的重复行
        sort -u {output.readlist} -o {output.readlist}.dup
        rm {output.readlist}
        mv {output.readlist}.dup {output.readlist}
        """
...
rule flye:
    ...
    shell:
        ...
        flye -o {params.flye_out} --plasmids --threads {threads} \
         --pacbio-raw {output.long_filter}  -i 3
...
rule nextpolish:
    input:
        flye_genome=lambda wildcards: "{work_path}/output/flye/{sample}/assembly.fasta".format(work_path=work_dir, sample=wildcards.sample),
    ...
    output:
        genome="{work_path}/output/nextpolish/{sample}/{sample}.fasta"
    params:
        out_path="{work_path}/output/nextpolish/{sample}",
    ...
    shell:
        """
        cd {params.out_path}
        touch {params.sample}.cfg
        echo "[General]" > {params.sample}.cfg
        echo "job_type = local" >> {params.sample}.cfg
        echo "job_prefix = nextPolish" >> {params.sample}.cfg
        echo "task = best" >> {params.sample}.cfg
        echo "rewrite = yes" >> {params.sample}.cfg
        echo "rerun = 3" >> {params.sample}.cfg
        echo "parallel_jobs = 1" >> {params.sample}.cfg
        echo "multithread_jobs = 6" >> {params.sample}.cfg
        echo "genome = {input.flye_genome}" >> {params.sample}.cfg
        echo "genome_size = auto" >> {params.sample}.cfg
        echo "workdir = {params.out_path}/" >> {params.sample}.cfg
        echo "polish_options = -p {{multithread_jobs}}" >> {params.sample}.cfg
        echo "" >> {params.sample}.cfg
        echo "[sgs_option]" >> {params.sample}.cfg

        ls {input.fq1} {input.fq2} > sgs.fofn
        echo "sgs_fofn = ./sgs.fofn" >> {params.sample}.cfg
        echo "sgs_options = -max_depth 100 -bwa" >> {params.sample}.cfg
        nextPolish {params.sample}.cfg
        mv {params.out_path}/genome.nextpolish.fasta {output.genome}
        test -f {output.genome}
        """

rule checkm:
    input:
        genome=lambda wildcards: "{work_path}/output/nextpolish/{sample}/{sample}.fasta".format(sample=wildcards.sample, work_path=work_dir)
    output:
    ...
    wildcard_constraints:
        sample="|".join(samples)
    params:
        input_path="{work_path}/output/nextpolish/{sample}",
...
rule quast:
    input:
        genome=lambda wildcards: "{work_path}/output/nextpolish/{sample}/{sample}.fasta".format(sample=wildcards.sample, work_path=work_dir),
        long_fastq=lambda wildcards: "{work_path}/output/cleanlong/{sample}.fastq".format(sample=wildcards.sample, work_path=work_dir)
    output:
    ...
    shell:
        ...
        quast {input.genome} -t {threads} -r {params.ref} -m 400 \
         --pacbio {input.long_fastq} -o {params.output}
        """
...
rule parsnpANDnucmer:
    input:
        genome=lambda wildcards: "{work_path}/output/nextpolish/{sample}/{sample}.fasta".format(work_path=work_dir,sample=wildcards.sample),
    output:
